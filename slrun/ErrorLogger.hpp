...
SL_ERROR_MESSAGE(syntax_error, E0003, (const std::string& what), "Syntax error. Exprected " << what)
SL_ERROR_MESSAGE(function_not_found, E0004, (const std::string& what), "Function not found: " << what)
SL_ERROR_MESSAGE(void_variable, E0005, (const std::string& name), "Cannot declare a variable of type void: " << name)
SL_ERROR_MESSAGE(no_variable_type, E0006, (const std::string& name), "Cannot declare a variable with no type: " << name)
SL_ERROR_MESSAGE(not_convertible, E0007, (const std::string& from)(const std::string& to), "Cannot convert an expression of type " << from << " into type " << to)
